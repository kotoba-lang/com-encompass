(ns encompass.main
  "Kotodama WASM entrypoint for the Encompass clean-room actor (L4) — Clojure port.
  ...
  facts (`encompass.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "encompass")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "encompas_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}               :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "encompas_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"encompass.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "encompass-compat" :tier tier :entities entities} 200])
...
(defn start! [] :encompass-compat/ready)
